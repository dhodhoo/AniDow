...
export default function PlayNowButton({ animeSlug, episodes }: PlayNowButtonProps) {
  ...
  return (
    <Link href={targetEpisode ? `/watch/${targetEpisode}` : "#"} aria-disabled={!targetEpisode} className="w-full sm:w-auto">
      <button
        className="flex w-full sm:w-auto items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-500 hover:scale-105 active:scale-95 transition-all duration-300 text-white px-8 py-3.5 rounded-full font-black text-sm shadow-[0_0_20px_rgba(99,102,241,0.5)] border border-indigo-400"
      >
